        ...
        <div id='moviesearched'>
            <div class='movie-info'>
                <?php if (!empty($movieData)): ?> <!-- if queried movie is found then display the info -->
                    ...
                    <button id="add-to-watchlist" data-movieid="<?= $movieData[0]['id']; ?>" data-moviename="<?= htmlspecialchars($movieData[0]['title']); ?>">Add to Watchlist</button>
                    <script>
                        document.getElementById('add-to-watchlist').addEventListener('click', function() {
                            var movieId = this.getAttribute('data-movieid');
                            var moviename = this.getAttribute('data-moviename');
                            var xhr = new XMLHttpRequest();
                            xhr.open('POST', '/add-to-watchlist', true);
                            ...
                            xhr.send('movieid=' + movieId + '&moviename=' + encodeURIComponent(moviename));
                        });
                    </script>
                <?php else: ?>
